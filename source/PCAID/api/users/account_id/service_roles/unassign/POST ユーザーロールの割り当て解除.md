...
```http
/users/{account_id}/service_roles/unassign
```

## 説明

指定したPCAアカウントに対して、紐付けられたサービス区画用[ロール](/docs/common/ロール（役割）.md)の割り当てを解除します。

- 同じ[ロール](/docs/common/ロール（役割）.md)に対して、割り当てが解除済みなら何もしない
...
| Field_Name | Field_Name_Description | Required | Type | Comment |
| -- | -- | -- | -- | -- |
| service_roles | サービスロール一覧 | required | String | ・指定されたサービス区画用ロール（X-PCA-service-partitionと結合する）についてユーザーへの割り当てを解除する <br/>・例：`pca.hub.tdi/gs:admin` |
