...
```http
/users/{account_id}/lifecycle/unsuspend
```
...
- Keycloakユーザーを有効にする
  - サービス内からはロックアウトされたユーザーに対する解除操作に相当する
  - 有効となったPCAアカウントは、利用可能なすべてのサービスを再び使えるようにになる
- Keycloak API の `PUT /users/{id}` で `enabled` フィールドを `true` に更新する
